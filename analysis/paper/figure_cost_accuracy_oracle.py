...
# ///
"""5 model x 4 benchmark cost-accuracy grid focused on the two oracles.

Companion to ``figure_cost_accuracy_full.py``: same 5x4 layout but
only four curves (Standard MV / PC-cubic / their two oracles) so the
oracle gap is the visual focus. The y-axis upper bound includes the
oracle plateau, unlike the all/full variants.
...
# Solid baselines first, oracles on top so their dotted strokes
# stay visible where curves overlap.
DRAW_ORDER = [
    "standard_mv",
    "prefix_cubic",
    "oracle_init",
...
def _style_for(key: str) -> dict:
    ...
    if key in ORACLE_KEYS:
        return dict(color=color, lw=1.3, ls=(0, (1, 1.6)),
                    zorder=10, alpha=0.95)
    return dict(color=color, lw=1.6, ls="-",
                zorder=6 if key == "prefix_cubic" else 4,
                alpha=0.95)
...
def _draw_panel(ax, jsonl_dir: Path, condition: str) -> bool:
    ...
    for key in DRAW_ORDER:
        ...
        drew = True
        # Include oracles so auto-zoom covers their high-budget tail.
...
LEGEND_ENTRIES = [
    ("Standard MV", "standard_mv"),
    ("PC-cubic (ours)", "prefix_cubic"),
    ("Oracle (Standard MV)", "oracle_init"),
    ("Oracle (Prefix Consistency)", "oracle_prefix"),
]


def main():
    parser = argparse.ArgumentParser(
        description="5x4 cost-accuracy grid focused on the two oracles")
    ...
    parser.add_argument("--stem", default="cost_accuracy_grid_oracle")
    ...
    for label, key in LEGEND_ENTRIES:
        st = _style_for(key)
        handles.append(Line2D([0], [0], color=st["color"], lw=st["lw"],
                              ls=st["ls"]))
        labels.append(label)
